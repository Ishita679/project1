...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!passwordRegex.test(password)) {
      return toast.error("Password must be at least 8 chars, contain an uppercase, lowercase, number, and special character.");
    }
    ...
    <div className="min-h-screen bg-[var(--color-bg-primary)] flex items-center justify-center relative overflow-hidden bg-noise">
      ...
      <div className="glass-card w-full max-w-md p-8 relative z-10 mx-4">
        <div className="flex flex-col items-center mb-8">
          ...
          <h2 className="text-3xl font-serif font-bold text-[var(--color-text-primary)] tracking-tight">Create Account</h2>
          <p className="text-[var(--color-text-secondary)] text-sm mt-2 font-light">Join TubeMind to start learning</p>
        </div>

        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          <div className="flex flex-col gap-2">
            <label className="text-xs font-semibold tracking-widest uppercase text-[var(--color-text-secondary)]">Full Name</label>
            <input 
              type="text" 
              value={name}
        ...
        <p className="mt-6 text-center text-sm text-[var(--color-text-secondary)]">
          Already have an account? <Link to="/login" className="text-[var(--color-accent-gold)] hover:underline">Sign in</Link>
        </p>
      </div>
    </div>
  );
